...
export const FriendShipModel = {
    async friendRequestSend(requesterId, addresseeEmail) {
    ...
    async friendRequestAccept(friendshipId) {
    ...
    async friendRequestReject(friendshipId) {
    ...
    async cancelFriendRequest(friendshipId) {
        const [deleted] = await db.delete(user_friendships)
            .where(and(eq(user_friendships.id, friendshipId), eq(user_friendships.status, 'pending')))
            .returning();
        return !!deleted;
    },
    async getFriendsList(userId) {
        const friends1 = await db.select({
            ...
            .where(and(eq(user_friendships.requesterId, userId), eq(user_friendships.status, 'accepted')));
        const friends2 = await db.select({
            ...
            .where(and(eq(user_friendships.addresseeId, userId), eq(user_friendships.status, 'accepted')));
        return [...friends1, ...friends2];
    },
    async getFriendRequestsList(userId) {
        const requests = await db.select({
            ...
            .from(user_friendships)
            .innerJoin(usersTable, eq(user_friendships.requesterId, usersTable.id))
            .where(and(eq(user_friendships.addresseeId, userId), eq(user_friendships.status, 'pending')));
        return requests;
    },
